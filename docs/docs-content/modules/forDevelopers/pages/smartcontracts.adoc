...
==== Inherited structs from `BaseConfigAdmin.sol`
...
== `SBCoin.sol`
The `SBCoin.sol` smart contract is the contract which stores the _Knowledge Coin_ in short _NOW_. This coin is the main currency of the Knowledge Base system and the payment students receive for successfully passing tests on their submitted assignments.
...
The `burn` function can burn Knowledge Coins from the student's address. This function is also restricted to user or contract admins. Coins are burned by transferring them to a 0x0 address using the `_transfer` function.
...
Each assignment validator, for convenience we call it _validator_, is pack of different contracts.
...
* Deployed and submitted in block range of the assignment (plus 6 days of grace period)
...
The `BaseValidator.sol` contract contains the `hasFunction` function. This function takes the following parameters:
